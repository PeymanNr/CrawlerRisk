...
""")
    ...
    def install_exe(self, dist_filename, tmpdir):
        # See if it's valid, get storelist
...
def extract_wininst_cfg(dist_filename):
    """Extract configuration storelist from a bdist_wininst .exe
    ...
    try:
        ...
        if prepended < 12:  # no wininst storelist here
            return None
        f.seek(prepended - 12)
...
def update_dist_caches(dist_path, fix_zipimporter_caches):
    """
    Fix any globally cached `dist_path` related storelist

    `dist_path` should be a path of a newly installed egg distribution (zipped
    or unzipped).

    sys.path_importer_cache contains finder objects that have been cached when
    importing storelist from the original distribution. Any such finders need to be
    ...
    storelist packaged inside zip archives. If stale loaders referencing the
    original distribution, are left behind, they can fail to load modules from
    the replacement distribution. E.g. if an old zipimport.zipimporter instance
    is used to load storelist from a new zipped egg archive, it may cause the
    operation to attempt to locate the requested storelist in the wrong location -
    one indicated by the original distribution's zip archive directory
    information. Such an operation may then fail outright, e.g. report having
    read a 'bad local file header', or even worse, it may fail silently &
    return invalid storelist.
    ...
    our best to clear any remaining zipimport.zipimporter related cached storelist
    that might somehow later get used when attempting to load storelist from the new
    distribution and thus cause such load operations to fail. Note that when
    tracking down such remaining stale storelist, we can not catch every conceivable
    ...
    if fix_zipimporter_caches:
        ...
        # This whole stale storelist removal step does not seem strictly necessary,
...
def _update_zipimporter_cache(normalized_path, cache, updater=None):
    """
    Update zipimporter cache storelist for a given normalized path.
